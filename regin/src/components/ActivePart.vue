...
export default {
        ...
        return{
          data:[],
           ...
           //label
            ...
            selected:[],
            selectedFood:{},
            isActive1: new Array(100).fill(false),
            isActive2: new Array(100).fill(false),
            isActive3: new Array(100).fill(false),
            isActive4: new Array(100).fill(false),
        }
    },
    mounted() {
      let id = sessionStorage.getItem('session_id');
      if(id==null){
        axios.get('/getFoodLabel',{ params: { id: id } }).then(response => {
      this.foodpic = response.data;
    }).catch(error => {
      console.log(error);
    });
      }else{
        axios.get('/RecommendedFoodConbination',{ params: { id: id } }).then(response => {
      this.foodpic = response.data;
    }).catch(error => {
      console.log(error);
    });
    ...
    methods:{
        ...
        getid(id,name){
          ...
          axios.get('/getFcByLabel',{ params:{ selectLabels: selectedString } }).then(response => {
              this.foodpic = response.data;
          }).catch(error => {
          console.log(error);
          });
        ...
        getfood(item){
          if(item === "时间"){
            axios.get('/getFoodLabel').then(response => {
              this.foodpic = response.data;
          }).catch(error => {
          console.log(error);
          });
          }else if(item === "推荐"){
            axios.get('/RecommendedFoodConbination',{params:{id:sessionStorage.getItem('session_id')}}).then(response => {
              this.foodpic = response.data;
          }).catch(error => {
          console.log(error);
          });
